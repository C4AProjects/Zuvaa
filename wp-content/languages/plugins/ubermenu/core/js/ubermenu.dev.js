...
jQuery(document).ready(function($){
	...
	uberMenuSettings['windowsClick'] = uberMenuSettings['windowsClick'] == 'on' ? true : false;
	...
	if( uberMenuSettings['windowsClick'] && window.navigator.msMaxTouchPoints ){
		uberMenuSettings['trigger'] = 'click';
	}
...
;(function($) {
	...
	$.uberMenu = function(el, options) {
		...
		var init = function() {
							...
							function(e){
								...
								if( $li.hasClass( 'wpmega-expanded' ) ){
									...
									if( $(this).is( 'a' ) ){
										...
										if( href !== '' && href !== '#' ){
											if( e.target.className != 'uber-close' ){
			...
			//Windows 8 Touch Temporary Special - also add hover
			//Temporary fix code is redundant and will be unified in v3
			if( window.navigator.msMaxTouchPoints && uberMenuSettings['windowsClick'] ){
				$megaItems
					.hover( 
						function(){
							showMega( this );			
			...
			if( uberMenuSettings['iOScloseButton'] && ( jQuery.uber_mobile || is_iOS ) ){
				...
				var $navClose = $( '<span class="uber-close">&nbsp;</span>' );
				...
				plugin.$megaMenu.on( 'click' , '.uber-close' , function(e){
					...
					if( $( this ).attr( 'data-uber-status' ) == 'open' ){
						...
						$( this ).html( '&nbsp;' ).attr( 'data-uber-status' , 'closed' );
					}
					else{
						showMega( $(this).parents( 'li.mega-with-sub' )[0] );
						$( this ).html( '&nbsp;' ).attr( 'data-uber-status' , 'open' );
					}
				...
				plugin.$megaMenu.find('ul.megaMenu > li.mega-with-sub').hover(function(e){
					e.preventDefault();
					$( this ).find( '.uber-close' ).html( '&nbsp;' ).attr( 'data-uber-status' , 'open' ).show();					
				}, function(e){
					e.preventDefault();
