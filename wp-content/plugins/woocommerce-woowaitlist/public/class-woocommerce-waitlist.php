...
class Woocommerce_Waitlist {
	...
	public function woowaitlist_save_settings(){
		...
		switch( $data['action'] ){
			...
			case "update_woowaitlist_settings":
				...
				$wew_out_of_stock_str = trim( wp_unslash( $data['wew-out-of-stock-str'] ) );
				update_option( 'wew-out-of-stock-str', $wew_out_of_stock_str );
				...
				$based_on_subscription = trim( wp_unslash( $data['wew-based-on-subscription'] ) );
				update_option( 'wew-based-on-subscription', $based_on_subscription );
	...
	public function wew_email_onWaitlistAdd( $user_email, $product_id, $variation_id = 0, $gparent_id = 0 ){
		...
		$email_subject = get_option('wew-subscription-email-subject') ? get_option('wew-subscription-email-subject') : __( "%product% added to your Waitlist", $this->plugin_slug );
	...
	public function woowaitlist_shortcodes_filter( $content, $productId, $variation_id ){
		...
		$site_title = get_option( 'woocommerce_email_from_name' ) ? get_option( 'woocommerce_email_from_name' ) : false;
		...
		if( $url_email && is_email( $url_email ) && $url_productId ){
			...
			if( $delete ){
				...
				if( $url_gparentId ){
					...
					$d .= '<p>';
					$d .= __( 'Your email', $this->plugin_slug );
					$d .= ' <strong>' . $url_email . '</strong> ';
					$d .= __( 'removed from notification list for grouped product', $this->plugin_slug );
					$d .= '';
					$d .= __( '', $this->plugin_slug );
					$d .= ' ' . $parentLink;
					...
					$d .= __( 'Your email', $this->plugin_slug );
					$d .= ' <strong>' . $url_email . '</strong> ';
					$d .= __( 'removed from notification list for product', $this->plugin_slug );
					$d .= '';
					$d .= __( '', $this->plugin_slug );
					$d .= ' ' . $productLink;
	...
	public function wew_check_product_availability( $availability, $_product ){
		...
		if( !$disable_woowaitlist_subscription ){
			...
			$display_variationOutOfStock = $this->productStock_quantity( $_product->id, $_product->variation_id, $email_notification = false ) ? false : true;
			...
			if( $bo != 'no' ){
				...
				if( $isVariation ){
					...
					$quantity = intval( $this->productStock_quantity( $_product->id, $_product->variation_id, $email_notification = false ) );
				}
				else{
			...
			if( !$isVariation || ( $display_outOfStock && $this->get_hc() == 0 ) || ( $display_outOfStock && $this->get_hc() > 0 && $isGrouped ) ){
				...
				if( $display_outOfStock || $subscribe_backorders ){	// product has NOT stock
					...
					if( $subscribe_backorders ){
						...
						$display_msg = get_option('wew-out-of-stock-str') ? get_option('wew-out-of-stock-str') : __('Out of stock', $plugin_slug );
					...
					if( is_user_logged_in() ){
						...
						$tStr = $this->get_subscribe_button_str() != '' ? $this->get_subscribe_button_str() : __( "Subscribe", $this->plugin_slug );

						$display_form .= '<button id="wew-submit-email-to-notify" class="add_to_cart_button button">' . $tStr . '</button>';
					}
	...
	public function productStock_quantity( $pid, $vid = 0, $email_notification = false ){
		...
		$based_on_quantity = get_option('wew-based-on-subscription') == 'quantity' ? 1 : 0;

		if( $vid > 0 ){

			$variation_meta = get_post_meta( $vid );

			if( $based_on_quantity ){

				$quantity = isset( $variation_meta['_stock'] ) && isset( $variation_meta['_stock'][0] ) ? intval( $variation_meta['_stock'][0] ) : 0;

				if( $variation_meta['_manage_stock'][0] == 'yes' && $quantity > 0 ){
			...
			else if( get_post_meta($pid, '_stock_status',true) == 'instock' ){
			...
			$quantity = (int)get_post_meta($pid, '_stock',true) ;

			if( $based_on_quantity && get_post_meta($pid, '_manage_stock',true) == 'yes' && $quantity > 0 ){
	...
	private function send_backInStock_email( $email, $pid, $vid = 0, $gparent_id = 0 ){
		...
		if( is_email( $email ) ){
			...
			elseif( $gparent_id > 0 ){
				...
				$d = __( 'Your Grouped Child Product', $this->plugin_slug );
	...
	public function wew_wp_mail_from( $email ){
	    ...
	    $email = $inserted_email ? $inserted_email : get_option( 'woocommerce_email_from_address' ) ;
