...
class Woocommerce_Waitlist_Admin {
	 ...
	 * @var      string
	 */
	public function export_woowaitilist_data( $ids, $single = false ){
		...
		if( $ids ){
			...
			if( $single ){
				...
				$sql = "SELECT * FROM ".$wew_DBtable_name." WHERE productId IN (" . implode( ",", $ids ) . ") ORDER BY date DESC";
			}
			else{
			...
			foreach ( $ret as $k => $v ) {
				...
				if( ( !$single && in_array( $v->id, $ids ) ) || ( $single && in_array( $v->productId, $ids ) ) ){
	...
	public function execute_before_wp_header(){
		...
		elseif( isset( $_GET['export_woowaitlist_subscribers'] ) && isset( $_GET['post'] ) ){

			// var_dump( intval( $_GET['post'] ) );

			$this->export_woowaitilist_data( intval( $_GET['post'] ), true );
			...
			if( isset( $_POST['action'] ) ){
				...
				switch( $_POST['action'] ){
					case "update_woowaitlist_settings":
						...
						$wew_out_of_stock_str = trim( wp_unslash( $_POST['wew-out-of-stock-str'] ) );
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_woowaitlist_export_product_subscribers', array( $this, 'woowaitlist_export_product_subscribers' ) );
		add_action( 'wp_ajax_nopriv_woowaitlist_export_product_subscribers', array( $this, 'woowaitlist_export_product_subscribers' ) );
	}
	...
	public function enqueue_admin_scripts() {
		...
		if ( $screen->id == 'product' || isset( $this->plugin_screen_hook_suffix ) ) {

			if ( $screen->id == 'product' || $this->plugin_screen_hook_suffix == $screen->id ) {
	...
	public function woowaitlist_display_subscribers_meta_box( $post ){
		...
		echo '<div class="wew-expp-sub-wrapper"><a href="' . esc_url( add_query_arg( 'export_woowaitlist_subscribers', '1' ) ) . '" class="button button-primary button-large wew-export-products-subscribers">' . __ ( 'Export', $this->plugin_slug ) . '</a></div>';
	}
